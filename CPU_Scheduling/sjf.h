...
/* -------- SJF Core Algorithm (Non-preemptive) -------- */
void sjf(PCB input[], int n) {
        ...
        for (int i = 0; i < n; ++i) {
            if (!completed[i] && procs[i].at <= time) {
                if (procs[i].bt < min_bt || 
                   (procs[i].bt == min_bt && procs[i].at < procs[idx].at)) {
                    min_bt = procs[i].bt;
                    idx = i;
                }
            }
...
/* -------- SJF Averages (Non-preemptive) -------- */
void sjfaverage(PCB input[], int n) {
    print_header("SJF (Non-preemptive)");
    PCB *procs = copy_procs(input, n);
